...
Here's the list of things which should be done

1. Document the API properly so that official documentation and tutorials can be derived from it
	- tables(WEDNESDAY), 
	- data processing php(vanilla, cake, zend)(THURSDAY)
3. Complete the ToDo application as an example(NEXT WEEK)
	- implement local storage
4. Do unittest architecture and test mapping
5. Do templating system
6. Do the data binding
7. Write a plugin template as the starting point and checklist for plugins
8. Flesh out all the unit tests
9. Write up the website for publishing


After the project is completed and to be published
10. Get design for the website.
11. Add website community functionalty
12. Create package o download from git
13. Submit frameowrk to git

This will put the twoshoes project into a primary completion state.


DEVELOPMENT WORK TO PROPARE FRAMEWORK FOR PUBLISHING
This is the simple stuff which is not new functionality

- fix the table setting routine in the init() method
- fix the filters on the table property
	-set error if filter is not passed
- change query api "ngt" and "nlt" to be "gte" and "lte", makes more sense
- going to update the insert() to use an assocative array if user deson't want to use two arrays
	- might add the mass insert functionality using two dimensional values array.
	- using the insert function you should be ale to use the query refinements to selectively bulk insert
	- I think these bulk functions should use a new method import() or some similar name
		- this will need a similar function of say, export(), takes out data from the table and returns it as an array

ADDITIONAL FUNCTIONALITY TO ADD INTO THE FRAMEWORK
-Add an interface for model objects
-I want to be ale to save lookup queries as a variable and apply it back to the table functionality
...
NOTES ABOUT THE API:
	routes : {
		myLabel : { //property name of the label for use  in code
			event : 'onclick', //typ of event which will be dispatched
			target : '#myDiv', //class or id of the element which is the target of the event
			handle : '.myclass', //Optional attribute and value
			subject : '-> div.mydiv', //jquery ofthe subject of the event in dispatch
			qualify : ['-> yourDiv.yourClass', '<- <- hisDiv.hisClass'], //I can't remember what this is(), did I code this? Not implemented yet
			dispatch : function(){}; //code object to execute on event trigger

NEED TO ALSO ADD THE HASHBANG PROPERTY FOR SETTING THE URL ON THE BROWSER TO INDICATE THE LOCATION OF THE ROUTE ONCE
ONCEDISPATCHED
The other way of doing this is just to set the window location manually inthe dispatch.
	but it would be best for programaility to be able to set this as a property
	This would happen after the dispatch routine is fired.

I NEED TWO THINGS IN TERMS OF THE ROUTE.
	1. their needs to be a dependency before the dispatcher is fired, like a condition
	2. there needs to be a locsation that the route is set to once the dispatcher is fired.

I would like to make a function which lets the user clear the configuration objects which
have been set to the framework object.
This entails defining the type, group and namespace of the configurations to remove so the user cn clear
specific modules from the twoshoes config object while keeping everything else on. This needs to have simple
yet powerful ganularity as part of the config tear down process.
...
Events:
Need to be added to an event registry and that is to be looped through every time the event delegation is called
everytime a new event is added through the configuration setting it is put onto the evet registry and that event
	type is then reset with the new values on it.

I'd like to add a debug  mode which automatically prevents default behaviour like forms being submitted and
links being follwed o that the user can view bedug output of the router - this can be an input in the console area

I would like to add a listener/handler so that a plugin can dispatch n event o the framework.


Widgets:
Widgets are a registry of possible widgets o build.
The models for the widget need to be fleshed out a bit more
	Not sure whether to have event binding actions on the idget or the table
	I could simply leave that up to the event object dispatch to deal with binding events
To build widgets, evey value hich is inserted into the widget has to have a placeholder which allows
	the post building routine to go through the nodes on the widget and match those placeholders and assign
	the dom element to the bindings(this also needs to be se on a registry) which can hen be used
	as look up when an event is fired.
...
Models
add ability to use models which have behaviour which are like helpers but can package up events and utility into a singl wrapper to be called in routes/requests
I'm not entirely sure of the merit of this idea.


////////////////////////////////////////////////////////////////////////////////
API = this needs to be documented
...
//*!* Need to put some proper autoscript loadng into the framework, this will require a registry
//Call in function like this
//Twoshoes.require('dirname/scripname.js');
//all config types will have a require property for that block of code
//IE: require : 'dirname/scriptname'

Template tokens which are going to be used are the same as monkeywrench
{jQueryvariable}
{each(var as key to value on count)}
{jQuerykey}{jQueryvalue}{jQuerycount}
{/each}
{if(var eq value)}
{else}
{/if}
...
twoshoes.init(
	...
	routes : {
		myLabel : {
			...
			handle : '.myclass',
			subject : '-> div.mydiv',
			qualify : ['-> yourDiv.yourClass', '<- <- hisDiv.hisClass'],
			behave : true,
			dispatch : function(){
            	alert('hello');
			}
	...
	requests : {
		bootstrap : {
			myRequest : {
	        	...
	        	behave : false,
	        	method : 'post',
	        	values : [], //if thre are no values serialise the form
	        	format : 'json', //if the form is serialised do json by default
	        	error : function(){
	            	//alert('sorry');
				},
	        	response : function(values){ //not sure how to pass this
					//alert(values['message']);
					Twoshoes.table('myTable').update(values)
				}
			}
		}
	},
	...
	widgets : {
		myLabel : {
        	...
        	target : '#yourDiv',
        	action : 'replace',
        	//tables : [myTable, youTable] //models takes care of this
        	models :  {//models will return an array and a simple call will return the first set - myModel.value = myModel[0].value
				myModel: 'join.myTable.hello.to.yourTable.world.where.myTable.%.eq.yourTable.id',
				someModel: 'join myTable.hello to yourTable.world where myTable.field > %',
        		yourModel: 'select myTable.myValue = yourTable.test.%' // i should really just do SQL in javascript
        	values : {//values are out do this {$myTable.myValue}
            	myTable : 'hello', myjoin, 'world'}
			}
		}
	},
	//define datasets
	tables : {
		myLabel : {
        	handle : '#myDiv', //i don't want t use this now
        	filters : {
            	myField : ['my filter rule'],
            	yourField : ['my filter rule']
			},
			fields : ['myNumber, yourNumber']
			values : [
				['one', 'two'],
				['three', 'four']
			]
	...
	//the call is simply Twoshoes.helper("myHelper").someFunction(param);
	helpers : {
		myHelper : {
...
//request calls
Twoshoes.request('groupname').property(modifiers);
//the property is the rquest configuration property.
//the modifiers object is merged with same config properties(EG: format:'json')
//to set a callback on the request set a callback function on a property such as complete, etc
//EG; the bootstrap could have a callback placed on the "complete" config property like this

var test = function myCompleteCallbackFunction()
{
	//Do the request, or something else.
...
Twoshoes.table(tableName).where(['or' => [[field > one], [field < four]]]).order('field').limit(10, 5).select()


//tested table calls
//var where = new Array();
//where['or'] = [['myNumber', 'eq', 'one'], ['yourNumber', 'eq', 'six']];
//var results = Twoshoes.table('myTable').where(where).limit(1).order('myNumber', 'desc').extract('yourNumber', false).select(true);
//var results = Twoshoes.table('myTable').where(where).limit(1, 1).update(['five', 'six'], ['myNumber', 'yourNumber'], true);
//var results = Twoshoes.table('myTable').insert(['six'], ['yourNumber'], true);
//var results = Twoshoes.table('myTable').where(where).remove(true);


//The link functionality needs to be iplemented.
//var results = Twoshoes.table('myTable').where(where).limit(1, 1).link(name, [rows]);
//this is done and example here works, link those users to the address on row one
//	Twoshoes.table('user').limit(1).order('created', 'asc').link('address', 1);
...
{tables: {
	user : {
		...
		values : [
	    	['myname', 'name@address.com'],
	    	['yourname', 'email@address.com']
		]
		errors : [
			['username incorrect', 'email incorrect']
		]
		links : [
			['type', 'key']
		]
...
//Actions for the table
{
	...
	values:{name:"name",type:"parcelname",row:"index"}
}
//The link value needs to contain mulitple links, the good news is I don't think there are multiple links in this website.
//The best thing we can do is create a routine that abstracts the get parcel data in the backend api and match that in the front end.
//I'm not sure the api will ever put out data with multiple linked items in it.
//I'm going to ignore it at the crent step in proceedings.

//Here is the "nice to have" plan which I don't have time to implement now.
Have the action just be an array list which looks something like this;
{
	0 : {type:insert;values:{field:whatever}}
	1 : {type:update;values:{field:whatever}}
	2 : {type:link;type:parcelname,row:index;}
	3 : etc
}

this lets you take what the user what the users are doing before th data is recieved by the backend.

Need to fgure out what the vlue registry is for nd whether or not to use it.
At the moment we are making modifications directy to the config data not the registry.
//Shall we use that to remember what the value  once were?

The following is the current arbitrary way of storing array type information as a string within a table field
key_value|
This will probably change if a routine can be found which makes more sense in handling array-like data
such as jon conversion which is not bing used because it becomes cumbersome in the backend.
The following functions are doing the conversaiton
unserialise()
serialise()

with the route events I would like to be able to define multiple types of events on a single rout such as
event : 'onclick,onblur'
also need to address keypress to match for a particular key being pressed
...
var Twoshoes = {
	...
	//Executable background task as a function.
	background					: {},
	//Background task interval variable.
	interval					: 0,
...
//	routeEvents					: ['click', 'focusin', 'focusout', 'keypress', 'hover', 'keydown', 'keyup', 'mouseleave', 'mousemove', 'mouseout', 'mouseover', 'mousedown', 'mouseup', 'resize', 'change', 'select'],
	...
	init : function(config)
	{
		...
		if (config)
		{
			...
			//Setup background task heartbeat.
			if (typeof config.heartbeat != 'undefined')
			{
				if ((config.heartbeat != 0) && (config.heartbeat != false))
				{
					if (!Twoshoes.isInteger(config.heartbeat))
					{
						//do error, value not an int
						Twoshoes.heartbeat = 0;
					}
					else
					{
						Twoshoes.heartbeat = config.heartbeat;
					}
				}
				else
				{
			...
			//Set background task function.
			if (typeof config.background != 'undefined')
			{
				if (Twoshoes.isFunction(config.background))
				{
					Twoshoes.background = config.background;
				}
			}
...
//	 				else
//	 				{
//	 					indexes = Twoshoes.eventRegistry[eventsList[i]].length;
//	 				}
...
//						jQuery(document).off();
					...
					//*!*There is an issue with this event restration
					else if (eventsList[n] == 'mouseup')
					{
//						jQuery(document).off();
						jQuery(document).on(eventsList[n], {type:eventsList[i], routes:Twoshoes.eventRegistry[eventsList[n]], builder:builder}, Twoshoes.dispatch);
					}
	...
	// * Return:				SELF
	//*!*Implement the match parameter, check the rest of the routines.
	...
	node : function(position)
	{
		...
		else if (Twoshoes.isString(location.hash) && location.hash == '')
		{
			return '';
		}
	...
	//*!*We're not getting a dispatch on multiple maches in the route.
	...
	// - label:				Title of the debugging entry for code location reference
...
//Still need to figure out a system to index and reference the bound data.
	...
	//*!*We're not able t deal with nested arrays, EG;  [["model_name", "eq", "cycle"], ["model_id", "eq", "c4ca4238a0b923820dcc509a6f75849b"]]
	//this is two conditions which should not be passed to this function.
	isCondition : function(condition)
	{
		...
		else
		{
			...
			{
				//*!*High priority fx.
				//*!*Removing this test, it must be more difinitive to test that the values inside the three array data pieces are scalar
				//so logic is: condition.length == 3 && (loop through with all values being scalar.)
	...
	isWhere : function(indexedRow, conditions)
	{
		...
		//If there are empty or no conditions set as passed.
		if ((typeof conditions == 'undefined')
		|| ((Twoshoes.isArray(conditions)) && (conditions.length == 0)))
		{
			return true;
		}
...
// console.log(splitValues);
	...
	link : function(linkTable, linkRow, memory)
	{
		...
		var rowsToLink = [];
		if (Twoshoes.isBoolean(linkRow))
		{
			rowsToLink = Twoshoes.affected;
		}
		else if (Twoshoes.isInteger(linkRow))
		{
			rowsToLink[0] = linkRow;
		}
		else if (Twoshoes.isString(linkRow))
		{
	...
	//*!*Going to add a second paramter to set a function to the current backround interval
	//	in addition to the configurable function set through the init()
	...
	{
		var setBackgroundTask = false;

		//If the period is boolean set switch for background tasks.
		if (Twoshoes.isBoolean(period))
		{
			window.clearInterval(Twoshoes.interval);
			if (period == true)
			{
				//Only set timer wth interval, without interval background task will be fired just this once.
				if (Twoshoes.heartbeat > 0)
				{
					period = Twoshoes.heartbeat;
		...
		//Else if the period is updated set new interval.
		else if ((Twoshoes.isInteger(period)) && (period != Twoshoes.heartbeat))
		{
			window.clearInterval(Twoshoes.interval);
			Twoshoes.heartbeat = period;
		...
		//Otherwise produce error and kick from function.
		else if (!Twoshoes.isInteger(period))
		{
			Twoshoes.debug('(E)backgroundTask.period', 'period is not valid', Twoshoes.debugError);
			return;
		}

		//Set background task interval.
		if (setBackgroundTask == true)
		{
			Twoshoes.interval = window.setInterval(function(){Twoshoes.backgroundTask(period)}, period);
		}
...
///////////////////////////////////////////////////////////////////////////////
	...
	//*!*Need to isolate the native javascript objects like Date, Math, etc.

	//*!*Add to documentation, not sure I want or need this function. Might put it in a helper
	//Tests variable is a valid property name.
	// - variable:				String or number to test as property name
	// * Return:				True if variable is a valid property name, otherwise false
	...
	//*!*This function is buggy, getting a number undefined error in some circumstances.
...
//Reset all dom manipulation events - do all funtions that modify the dom
// (function()
// {
//     // Store a reference to the original remove method.
//     var jQueryAttr = jQuery.fn.attr;
//
// 	// Define overriding method.
// 	jQuery.fn.attr = function()
// 	{
// 		//Update data bindings.
//     	Twoshoes.updateBindings(jQuery.get());//the get  method is te fuckup, that was supposed to be the taret object
//
