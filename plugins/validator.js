...
Added ability to pass false into error and valid message field properties
Added ability to pass functions error and valid, return value is the message string



NEED TO CHANGE

Urgent, need to add control o only validate a single fieldset or list of inputs as the user moves through the form.
***change***
Added validateFieldset function
Added validateFieldlist function
Added return value functionality inthe jQuery plugin layer



the attribute is being baddly used, look for alternative which wll not show to the user when hovering
	ove messages or field names. This might have to be html5 only feature
	Will probably use a data: psuedo attribute(though i don't want to do this)
Need to pull out some of this hard markup creating routines
...
(function (jQuery) {
	...
	jQuery.Validator = function (options, element, callback)
	{
		...
		this.opts = jQuery.extend(true, {}, jQuery.Validator.settings, options);
	...
	jQuery.Validator.settings = {
		...
		formClasses: {
			...
			// could do a jQuery(".myfieldset").find("."+opts.formClasses.requiredClass).each(function(){jQuery.fn.validator.validateField(jQuery(this),opts)}) on a click event or something;
	...
	jQuery.Validator.prototype = {
		init : function() {
			...
			this.theFormValidationFields.each(function ()
			{
				...
				{
					jQuery(this).addClass(validator.opts.formClasses.fieldActive);
				});

				jQuery(this).blur(function ()
				{
					jQuery(this).removeClass(validator.opts.formClasses.fieldActive);
					jQuery(this).removeClass(validator.opts.formClasses.fieldActiveValid);
					jQuery(this).removeClass(validator.opts.formClasses.fieldActiveInvalid);
					// if this field has a value then validate it
					if (jQuery(this).attr("value") !== "")
					{
						validator.validateField(jQuery(this));
					}
					// if the field is required valdate it
					else if (jQuery(this).hasClass(validator.opts.formClasses.requiredClass))
					{
						validator.validateField(jQuery(this));
					}
				});
				...
				jQuery(this).keyup(function ()
				{
					...
					if (jQuery(this).attr("value") !== "" && validator.opts.onChangeValidation == true)
					{
						validator.validateField(jQuery(this));
					}
				});
			...
			this.el.submit(function ()
			{
				...
				{
					//If there are no errors find submit value from options callback function.
					var submitFromCallback = true;
					if (validator.opts.callback != false)
					{
						//If the ccallback is not a function do error.
						if (Object.prototype.toString.call(validator.opts.callback) != '[object Function]')
						{
							alert('Validation callback is not a function');
						}
						else
						{
							//Get callback submit behaviour value.
							submitFromCallback = validator.opts.callback(validator);

							//Prevent the submit function from being called again.
							//*!*Not sure why this is happening given the strucure of the code.
							jQuery(this).unbind('submit');
						}
					}

					//*!*This still needs to be configured in the plugin to work at prevnting default or not.
					return submitFromCallback;
				}
				else
				{
		...
		{
			// get the element DOM object
			var element;
			fieldset.each(function(key, value)
			{
				element = value;
			});
			...
			validator.opts.errorCount = 0;
			validator.theFormRequired.each(function (i, n)
			{
				// check all parent fieldsets to match current tested fieldset to be validated
				var parentElements = jQuery(n).parents('fieldset');
				if (parentElements.length > 0)
				{
					parentElements.each(function(key, value)
					{
						if (value == element)
						{
							//console.log("validating field: "+theFormRequired.attr("id"));
							validator.validateField(jQuery(n));
						}
					});
				}
			});
		...
		validateFieldsByName : function (fieldList)
		{
...
//alert(field.attr("name"));
			...
			for (i in fieldRules)
			{
				...
				if (fieldRules.hasOwnProperty(i))
				{
					...
					case "checkboxMin":
						...
						checkedInField = jQuery("input[name=" + fieldName + "]:checked");
					...
					case "passConfirm":
						...
						//this is not a good test in mulit password forms
						var thisTitle = field.attr("title"),
							passField = jQuery("input[type='password'][title=" + thisTitle + "][name!=" + fieldName + "]"),
					...
					case "date":
						...
						{
							// get the input value as an array and then pass it to isDateValid to test if it is valid. Need to test that jQuery.fn.validator.getDateArray is not false before running isDateValid
					...
					case "dateFuture":
						...
						{
							//alert("finding rule");
							// check logic of this and 'past' test. Need to test that jQuery.fn.validator.getDateArray is not false before running dateTense
							dateArray = this.getDateArray(fieldValue);
					...
					case "datePast":
						if (fieldRules[i] === true)
						{
							// need to test that jQuery.fn.validator.getDateArray is not false before running dateTense
							dateArray = this.getDateArray(fieldValue);
					...
					if (hasError === true)
					{
						...
						if (Object.prototype.toString.call(fieldErrorMessage) === '[object String]')
						{
						...
						else if (Object.prototype.toString.call(fieldErrorMessage) === '[object Boolean]')
						{
							if (fieldErrorMessage)
							{
								message = this.opts.formClasses.fieldErrorMessage;
							}
						}
						else if (Object.prototype.toString.call(fieldErrorMessage) === '[object Function]')
						{
						...
						jQuery("." + this.opts.formClasses.errorClass + "[title='" + fieldName + "']").html(message);
					...
					else
					{
						...
						if (Object.prototype.toString.call(fieldValidMessage) === '[object String]')
						{
						...
						else if (Object.prototype.toString.call(fieldValidMessage) === '[object Boolean]')
						{
							if (fieldValidMessage)
							{
								message = this.opts.formClasses.validMessage;
						...
						else if (Object.prototype.toString.call(fieldValidMessage) === '[object Function]')
						{
							message = fieldValidMessage(field);
							if (!message)
							{
								message = '';
							}
						...
						jQuery("." + this.opts.formClasses.errorClass + "[title='" + fieldName + "']").html('<span class="' + this.opts.formClasses.validClass + '">' + message + '</span>');
		...
		option : function(args) {
			this.opts = jQuery.extend(true, {}, this.opts, args);
		},
		destroy : function() {
	...
	jQuery.fn.validator = function(options, callback) {
		...
		//*!*This is not very goog the test should be for not a function
		if ( typeof options === 'string' ) {
			...
			this.each(function() {
				...
				var pluginInstance = jQuery.data(this, pluginName);
				...
				if (!jQuery.isFunction(pluginInstance[options]) || options.charAt(0) === "_") {
					alert("the plugin contains no such method: " + options);
					return;
				}
				// apply the method that has been called
				else {
					// if there is a return value then get that
					var valueReturn = pluginInstance[options].apply(pluginInstance, args);
		...
		// initialise the function using the arguments as the plugin options
			...
			this.each(function() {
				...
				// initialise a new instance of the plugin
					...
					jQuery.data(this, pluginName, new jQuery.Validator(options, this, callback));
				}
			});
		}

		// not going to stack results in array so last value will override previous
